...
... class Parent:
...   name: Optional[str] = kw_only_dataclasses.field(default='', kw_only=True)
...
(self, size: float, name: Optional[str] = '')
...
def field(*, metadata=None, kw_only=dataclasses.MISSING, **kwargs):
  ...
  if kw_only is not dataclasses.MISSING and kw_only:
    if (kwargs.get('default', dataclasses.MISSING) is dataclasses.MISSING and
      ...
      raise ValueError('Keyword-only feilds with no default are not supported.')
    if metadata is None:
      metadata = {}
  ...
  for name, annotation in cls.__annotations__.items():
    if annotation is KW_ONLY:
      ...
      if not hasattr(cls, name):
        raise ValueError(
            'Keyword-only feilds with no default are not supported.')
